...
> > mass <- 47.5
> > ```
> > Este comando atribui o valor 47.5 para a variável `r mass` for the variable mass
> >
> > ```{r ch2pt2-sol}
> > age <- 122
> > ```
> > Este comando atribiu o valor 122 para a variável `r age` 
> >
> > ```{r ch2pt3-sol}
> > mass <- mass * 2.3
> > ```
> > O valor da variável `r mass`será dividido por 2.3 e a variável `r mass` passará a ter um novo valor.
> > 
> >
> > ```{r ch2pt4-sol}
> > age <- age - 20
> > ```
> > Será realizada uma subtração de 20 unidades na variável `r age`.
> > 
> {: .solution}
{: .challenge}
...
> > mass > age
> >```
> > O resultado a ser retornado deve ser `TRUE` porque a variável `r mass` é maior que `r age`.
> {: .solution}
{: .challenge}
